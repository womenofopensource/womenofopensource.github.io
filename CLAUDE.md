...
- **GitHub Pages build parity** *(follow-up, non-urgent):* the legacy Pages build warns
  ...
  gem's Jekyll, so local ≠ prod. Production currently runs **Ruby 3.3.4 / github-pages 232 /
  Jekyll 3.10.0** (per pages.github.com/versions). Site still builds/deploys fine. Resolve by
  either switching the Gemfile to the `github-pages` gem and matching prod locally with Ruby
  3.3.x (currently on 2.6.10), or adding a custom GitHub Actions build workflow (which would
  also silence the Node-20 deprecation notice from GitHub's managed pipeline).
