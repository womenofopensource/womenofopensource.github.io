---
title: Dawn Foster
name: Dawn Foster
pronouns: she/her                         # optional (e.g. she/her, they/them, he/him)
slug: dawn-foster                        # URL: /speakers/ada-lovelace
tagline: Open Source Strategist
description: Dawn Foster speaks about open source strategy, sustainability, governance, and more.
featured_image: '/images/speakers/dawn-foster.png'
location: United Kingdom                  # country (used for the country filter)
languages:                                # optional
  - English
expertise:                                # topics (used for the expertise filter)
  - Open Source Strategy
  - Project Governance
  - Software Sustainability
  - Open Source Community
  - Contributor Strategy
  - Research
  - Project Health Metrics
  - Open Source Program Offices
profiles:                                 # all optional — only those you add are shown
  sessionize: https://sessionize.com/dawn-foster/
  linkedin: https://www.linkedin.com/in/dawnfoster/
  mastodon: https://hachyderm.io/@geekygirldawn
  bluesky: https://bsky.app/profile/geekygirldawn.bsky.social
  github: https://github.com/geekygirldawn
  website: https://fastwonderblog.com/
talks:                                    # optional — one entry per talk
  - title: "Strategic Approach to Demonstrating the Value of Open Source Efforts"
    abstract: "This talk will help you reason about how OSS efforts allow your organization to achieve its goals along with framing and communicating that value in ways that resonate with your leadership team. [Video](https://www.youtube.com/watch?v=uhnTkYTiv6o&list=PLbzoR-pLrL6p2URzlq8xlNtBhPspd2xpi)"
    delivered_at:
      - Open Source Summit NA 2026
      - CHAOSScon EU 2026
      - OSPOlogy Live Lyon 2025
  - title: "Power Dynamics, Rug Pulls, and Other Corporate Impacts on OSS Sustainability"
    abstract: "Learn more about the power dynamics and steps that you can take to make better decisions about which OSS projects to embrace. [Video](https://www.youtube.com/watch?v=1-2L9DLFc8A)"
    delivered_at:
      - SCALE 2026
      - Open Source Summit EU 2025
      - FOSSY 2025
      - Monkigras 2025
  - title: "Proactive Governance to Build Sustainable Open Source Projects"
    abstract: "Details about the importance of governance, how to define project governance, using governance as a pathway to leadership, creating an intentional culture, and making project ownership (e.g., individual, organization, foundation) decisions. [Video](https://www.youtube.com/watch?v=bSBmq2AUM18&list=PLbzoR-pLrL6p2URzlq8xlNtBhPspd2xpi)"
    delivered_at:
      - Open Source Summit NA, 2026
hero_image: '/images/speakers/hero-dawn-foster.jpg
---

Dr. Dawn Foster is an OSS strategy consultant. She is also on the board of CHAOSS, OpenUK, and the Software Stewardship Lab. She is a TODO Group Ambassador and was previously a co-chair of the CNCF Contributor Strategy Technical Advisory Group. She has 20+ years of experience at companies like VMware and Intel with expertise in community, strategy, governance, and metrics. She has spoken at over 100 industry events and has a BS in computer science, an MBA, and a PhD. In her spare time she enjoys reading science fiction, running, 3D printing, and traveling.

You can see all of her past talks at https://fastwonderblog.com/speaking/
